...
public class CalendarClient extends BaseOfficeClient {
	 ...
	 * @return the event
	 */
	public Event newEvent(Calendar calendar) {

		/*
		if (calendar == null) {
			throw new IllegalArgumentException("calendar cannot be null");
		}
		Event event = Events.newEvent(calendar);
		return event;
		*/
		
		Event event = container.newEntityInstance(Event.class);
		event.setSubject("Testing");
		...
		ac.add(a);
		event.setAttendees(ac);
		
		event.operations().accept("some comment");
