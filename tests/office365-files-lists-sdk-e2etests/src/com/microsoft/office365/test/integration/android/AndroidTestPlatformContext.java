...
public class AndroidTestPlatformContext implements TestPlatformContext {
	...
	public MailClient getMailClient() {
		MailClient result = null;

		if (getAuthenticationMethod().equals("AAD")) {
			result = getMailClientAAD();
		} else {
			//result = getFileClientCookies();
		}
		...
		try {
			getAuthenticationContext().acquireToken(
					...
					new AuthenticationCallback<AuthenticationResult>() {
						...
						@Override
						public void onSuccess(AuthenticationResult result) {
							com.microsoft.office365.oauth.OAuthCredentials credentials = new com.microsoft.office365.oauth.OAuthCredentials(result.getAccessToken());

							MailClient client = new MailClient.Builder().setCredentials(credentials).setOdataEndpoint(Constants.ODATA_ENDPOINT)
